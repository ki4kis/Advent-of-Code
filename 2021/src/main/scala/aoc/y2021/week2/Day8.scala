...
object Day8 extends AdventApp[List[String]](year = 2021, day = 8) {
  def part1(input: List[Input]): IO[Any] = IO {
    ...
    val searching =
      Set(Digit.One, Digit.Four, Digit.Seven, Digit.Eight).map(_.segments.size)
...
object Digit {
  ...
  private val allPatterns: List[Pattern] = allSegments.permutations.toList map {
    result =>
  ...
  val Values: Set[Digit] =
    Set(Zero, One, Two, Three, Four, Five, Six, Seven, Eight, Nine)
  val Map: Map[Set[Char], Digit] =
    Values.map(digit => digit.segments -> digit).toMap
