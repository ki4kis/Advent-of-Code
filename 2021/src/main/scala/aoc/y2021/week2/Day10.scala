...
object Day10 extends AdventApp[String](year = 2021, day = 10) {
  ...
  def part2(input: List[Input]): IO[Any] = IO {
    val scores = (input zip input.map(fixedLine)).collect {
      case (left, right) if left == right.take(left.length) =>
        right.drop(left.length).foldLeft(0L) { case (score, char) =>
          score * 5 + incompleteScore(char)
        }
    }.sorted
  ...
  def fixedLine(line: String): String = {
    def loop(
        line: List[Char],
        expectedClosing: List[Char] = Nil,
        acc: List[Char] = Nil
    ): String = line match {
      case Nil => (acc.reverse ++ expectedClosing).mkString
      case (head @ ('(' | '[' | '{' | '<')) :: tail =>
        loop(tail, BracketMap(head) :: expectedClosing, head :: acc)
      case (')' | ']' | '}' | '>') :: tail =>
        loop(tail, expectedClosing.tail, expectedClosing.head :: acc)
    }
