...
case class HeightMap(map: Vector[Vector[Int]]) {
  ...
  def basins: List[Set[(Int, Int)]] =
    lowPoints.map(basinOf).toList.distinct.sortBy(_.size)

  private def basinOf(pos: (Int, Int)): Set[(Int, Int)] = basinOfLoop(
    LazyList(pos)
  )
  private def basinOfLoop(
      pos: LazyList[(Int, Int)],
      acc: Set[(Int, Int)] = Set.empty
  ): Set[(Int, Int)] = pos match {
    case _ if pos.isEmpty => acc
    case head #:: tail if acc(head) || getHeight(head) == 9 =>
      basinOfLoop(tail, acc)
    case head #:: tail =>
      basinOfLoop(tail #::: aroundPos(head).to(LazyList), acc + head)
  }
}

object HeightMap {
  def apply(input: List[String]): HeightMap = HeightMap(
    input.toVector.map(_.toVector.map(_.toString.toInt))
  )
}
