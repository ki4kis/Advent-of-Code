...
case class Vent(from: Pos, to: Pos) {
  ...
  private val vectorPos =
    (xList.padTo(length, xList.head) zip yList.padTo(length, yList.head)) map {
      case (x, y) => Pos(x, y)
    }

  def ventPos: List[Pos] = vectorPos.map { case Pos(x, y) =>
    Pos(from.x + x, from.y + y)
  }
