...
object Day4 extends AdventApp[Map[Field, String]](year = 2020, day = 4) {
  ...
  private def hasRequired(fields: Input): Boolean = Field.Required subsetOf fields.keySet
...
object Field {
  ...
  case object BYR extends Field { // Birth Year
    val Pattern = "(\\d{4})".r
    def isValid = {
      case Pattern(year) => isBetween(year)(1920, 2002)
    }
  }
  ...
  case object IYR extends Field { // Issue Year
    val Pattern = "(\\d{4})".r
    def isValid = {
      case Pattern(year) => isBetween(year)(2010, 2020)
    }
  }
  ...
  case object EYR extends Field { // Expiration Year
    val Pattern = "(\\d{4})".r
    def isValid = {
      case Pattern(year) => isBetween(year)(2020, 2030)
    }
  }
