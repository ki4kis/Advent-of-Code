...
object Day3 extends AdventApp[String](year = 2021, day = 3) {
  ...
  def rateSplitter(input: List[Input]): (List[Char], List[Char]) = input.map(_.toList).transpose.map { bits =>
  ...
  def ratingSplitter(input: List[Input], idx: Int = 0)(reduce: ((List[Char], List[Char])) => List[Char]): Input =
