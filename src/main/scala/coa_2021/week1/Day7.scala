...
object Day7 extends AdventApp[List[Int]](year = 2021, day = 7) {
  ...
  def fuelTo(to: Int)(from: Int)(implicit fuelCost: FuelCost): Int = math.round(fuelCost(math.abs(to - from))).toInt
  def fuelTo(to: Double)(from: Int)(implicit fuelCost: FuelCost): Double = fuelCost(math.abs(to - from))
