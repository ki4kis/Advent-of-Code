...
case class Vec(x: Int, y: Int) { self =>
  ...
  def sides(implicit edges: Edges): List[Vec]     = Vec.sides.map(self + _) filter edges.isWithin
  def corners(implicit edges: Edges): List[Vec]   = Vec.corners.map(self + _) filter edges.isWithin
  def allAround(implicit edges: Edges): List[Vec] = Vec.allAround.map(self + _) filter edges.isWithin
}
...
case class Edges(topLeft: Vec, bottomRight: Vec) {
  def isWithin(vec: Vec): Boolean = {
    def isBetween(lower: Int, upper: Int)(value: Int) = lower <= value && value <= upper
...
case class OctoMap(map: Map[Vec, Int], flashes: Int = 0) { self =>
  def steps(implicit edges: Edges): LazyList[OctoMap] = self #:: self.nextStep.steps
  ...
  private def reset: OctoMap = OctoMap(map.view.mapValues {
    case flashed if flashed > 9 => 0
    case energy => energy
  }.toMap, flashes + countFlashes)
