...
case class CaveMap(map: Map[String, List[String]]) {
  val allPaths1: List[List[String]] = loop(canTravelTwice = false)("start").sortBy(_.mkString)
  val allPaths2: List[List[String]] = loop(canTravelTwice = true)("start").sortBy(_.mkString)


  def loop(canTravelTwice: Boolean, pathTillNow: List[String] = Nil)(currentCave: String): List[List[String]] = {
    ...
    if (currentCave == "end") List((currentCave :: pathTillNow).reverse)
      ...
      if (canTravelTwice) map(currentCave).flatMap(loop(canTravelTwice = false, currentCave :: pathTillNow))
      else Nil
    else map(currentCave).flatMap(loop(canTravelTwice, currentCave :: pathTillNow))
  }
}
