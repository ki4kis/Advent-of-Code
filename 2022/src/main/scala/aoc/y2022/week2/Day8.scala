...
case class TreeGrid(grid: Vector[Vector[Int]]) { self =>
  override def toString = grid
    .map(_.map {
      case -1    => '+'
      case other => other.toString().head
    }.mkString)
    .mkString("\n")
  ...
  val part1: IO[Int] =
    allPos
      .parTraverse(pos => IO(visibleFromOutside(pos)))
      .map(_.count(identity))
  val part2: IO[Int] =
    allPos.parTraverse(pos => IO(visibilityInside(pos))).map(_.reduce(_ max _))
  ...
  def calculate(
      lineReduce: List[Int] => Int
  )(
      directionReduce: (Int, Int) => Int
  )(pos: Pos): Int = Vector(
  ...
  ).map(lineReduce).reduce(directionReduce)

  def visibleFromOutside(pos: Pos): Boolean = {
    val treeHeight = lookup(pos)
    val minHeight = calculate(_.reduce(_ max _))(_ min _)(pos)
    treeHeight > minHeight
  }

  def visibilityInside(pos: Pos): Int = {
    val treeHeight = lookup(pos)
    calculate(
      _.dropRight(1)
