...
object Day8 extends AdventApp[Operation](year = 2020, day = 8) {
  ...
  def part2(input: List[Input]): IO[String] = IO {
    ...
    def permutations(
        code: List[Operation],
        acc: LazyList[List[Input]]
    ): LazyList[List[Input]] = code match {
    ...
    def loop(pos: Int = 0, acc: Int = 0, used: Set[Int] = Set.empty)(
        code: Vector[Input]
    ): Int =
      if (end == pos) acc
    ...
    val allCodes =
      permutations(input.tail, LazyList(List(input.head))).map(_.toVector)
